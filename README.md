...
> 使用帮助见 [docs/用户使用指南.md](docs/用户使用指南.md)（系统内「帮助」按钮实时读取该文档）；各版本更新内容见 [docs/更新日志.md](docs/更新日志.md)（帮助面板「更新日志」页签实时读取）；完整功能规格见 [docs/系统功能说明书.md](docs/系统功能说明书.md)。
>
...
- **异常巡检**（P0-1）：数据源级巡检计划（每小时~每周）按期自动扫描最新真实数据报表，复用异常检测引擎（Z-Score/阈值）主动预警，含立即巡检、暂停/启用、运行历史与异常明细（严重度分级）；内置调度器多实例原子领取不重复执行；入口为系统管理「异常巡检」分类（v0.9.52 起统一入口，仅管理员）
...
- **账号创建校验对齐（v0.9.68）**：新建用户表单即时校验——用户名 3-20 位字母/数字/下划线（不支持中文）、初始密码 8-64 位且同时含字母和数字（非弱口令、不含用户名），规则与服务端 `validatePasswordStrength` 同源同步（`src/utils/passwordStrength.ts`），填写不合规时字段下方红色文案说明原因、按钮状态与提示一致（见说明书 2.1 节）
- **组织数据范围（v0.9.65）**：三层组织权限——用户档位（全辖 / 本机构 / 本项目团队 / 仅本人经办，`users.org_scope_json`）× 数据源组织列映射（机构列 / 团队列 / 责任人列，`data_sources.org_columns_json`）自动生成行过滤谓词，与数据源级 `rowFilters` 合并后由执行层 AST 强制注入（问数 / 下钻 / 报表 / 语义指标 / 情景推演 / Agent / 异步任务全链路生效，fail-closed）；结果缓存键含范围指纹防跨范围串用；存量未配置者行为不变（查询与配置见说明书 2.1 节）
- **组织架构树（v0.9.66）**：四级组织树（总部 → 机构 → 部门 → 团队，`org_units` 单表自关联）管理端维护（逐级折叠 / 展开收起 / 增删改 / 同级上下移）；用户「所属部门」从树中选择并派生 `users.department`（数据源 ACL / DLP 水印 / 审批留痕消费点零改动），改名自动同步用户部门与数据源 ACL 清单；节点「数据标识」供数据范围按树勾选自动生成档位取值（v0.9.67 起按层级路径自动编码——机构 `BR01` / 部门 `BR01-D01` / 团队 `BR01-D01-T01`，面板「补全数据标识」一键补全缺失节点，编码不随改名/移动变化）；删除保护（有下级节点或仍有归属用户拒绝）（见说明书 2.2 节）
- **组织数据范围自动派生（v0.9.69）**：未显式配置数据范围的用户按 `users.org_unit_id` 自动派生可见范围——部门 = 本部门 + 下辖全部团队、机构 = 本机构、团队 = 本团队、总部 = 不限制（`deriveOrgScopeFromUnit`）；鉴权时实时查 `org_units` 构建索引（`buildUnitScopeIndex`，BFS 收集后代数据标识），改组织树 / 改归属 / 改数据标识即时影响问数（无缓存）；显式配置优先（含显式全辖，`hasExplicitOrgScope` 区分「从未配置 NULL」与「显式 `{"level":"ALL"}`」），未绑定节点的存量用户行为不变；用户列表标注「（按所属组织）」、弹窗提示派生来源（见说明书 2.1 节）
...
| 测试 | Vitest（112 文件 / 1552 用例）+ NL2SQL 评测集（server/eval：主集 148 用例六类分层 + 行级权限类、机创集 54 用例、对比抽样集 60 用例；`npm run eval:seed` 一键重建可复现评测数据源；本地/云端模型对比见 [对比评估报告](docs/本地与云端模型问数对比评估报告20260914.md)） |
...
```bash
npm test             # Vitest（112 文件 / 1552 用例）
npm run lint         # TypeScript 类型检查
```

## 文档维护约定

`docs/系统功能说明书.md` 是系统功能的单一事实源（开发/评审侧）：每次功能新增或变更时同步更新该文件并在其「变更记录」章节追加一行；`docs/用户使用指南.md` 是终端用户侧操作向导，随功能同步维护，系统 Header 右上角「帮助」按钮实时读取展示（指南缺失时自动回退说明书），帮助面板另有「更新日志」页签实时读取 `docs/更新日志.md`（用户视角版本更新记录，随版本发布同步维护）。
