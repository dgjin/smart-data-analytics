...
- **组织架构树（v0.9.66）**：四级组织树（总部 → 机构 → 部门 → 团队，`org_units` 单表自关联）管理端维护（逐级折叠 / 展开收起 / 增删改 / 同级上下移）；用户「所属部门」从树中选择并派生 `users.department`（数据源 ACL / DLP 水印 / 审批留痕消费点零改动），改名自动同步用户部门与数据源 ACL 清单；节点「数据标识」供数据范围按树勾选自动生成档位取值；删除保护（有下级节点或仍有归属用户拒绝）（见说明书 2.2 节）
