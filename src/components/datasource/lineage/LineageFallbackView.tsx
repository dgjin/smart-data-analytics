...
/**
 * 血缘降级视图（血缘管理优化 P0 降级路径）：服务端 /api/lineage/graph 不可用时，
 * 沿用本地声明式三列推导（接入源层 → 逻辑提炼算子层 → 终端导向层）。
 * 三级兜底推导（固化 dataSourceId → 数据资源库推演 → 首个数据源）必须保留，不得删除。
 */
export const LineageFallbackView: React.FC = () => {
