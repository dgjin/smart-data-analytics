 ...
 * 通过后申请人 24h 内可重新导出一次（一次性授权，导出即消费）。
 */
...
const STATUS_META: Record<DownloadRequest['status'], { label: string; cls: string }> = {
  ...
  CONSUMED: { label: '已导出', cls: 'bg-slate-500/15 text-slate-400 border-slate-500/30' },
};

export const DlpDownloadPanel: React.FC = () => {
  ...
  const loadRequests = useCallback(async () => {
    try {
      const res = await apiFetch('/api/export/requests');
  ...
  const decide = async (r: DownloadRequest, action: 'approve' | 'reject') => {
    if (actingId) return;
    let note = '';
    if (action === 'reject') {
      const input = window.prompt(`驳回 ${r.username} 的导出申请（${r.rowCount} 行），可填写驳回原因（可选）:`);
      if (input === null) return; // 用户取消
      note = input.slice(0, 300);
    }
    setActingId(r.id);
    try {
      const res = await apiFetch(`/api/export/requests/${r.id}/${action}`, {
        method: 'POST',
      ...
      showNotice(
        'success',
        action === 'approve'
          ? `已通过 ${r.username} 的导出申请，24h 内可导出一次`
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-800">
        <div className="flex items-center space-x-2 text-xs font-bold text-slate-300">
          ...
          <span>
            数据导出审批（待审批 {pendingCount} / 共 {requests.length}）
          </span>
        </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-left text-slate-400 border-b border-slate-800 bg-slate-950/50">
              ...
              <th className="px-5 py-3 font-medium">导出内容</th>
              <th className="px-5 py-3 font-medium">行数</th>
              ...
              return (
                <tr key={r.id} className="border-b border-slate-800/60 text-slate-300 hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-5 py-3 max-w-[220px]">
                    <span className="block truncate" title={r.title}>{r.title || '未命名导出'}</span>
                    <span className="block text-[10px] text-slate-500">{r.dataSourceName || r.dataSourceId || '-'}</span>
                    {r.decideNote && (
                  ...
                  <td className="px-5 py-3 font-mono">{r.rowCount.toLocaleString()}</td>
              ...
              <tr>
                <td colSpan={7} className="px-5 py-10 text-center text-slate-500">
                  <div className="flex flex-col items-center space-y-2">
                    ...
                    <span>{showAll ? '暂无导出审批记录' : '暂无待审批的导出申请'}</span>
                  </div>
                </td>
              </tr>
            )}
