...
- [server/eval/evalRunner.test.ts](file://server/eval/evalRunner.test.ts)
</cite>

## 更新摘要
**所做更改**
- 更新了测试基础设施概览，反映测试文件从54个增长到90个、测试用例从569个增加到1035个的显著改进
- 增强了Vitest配置部分，详细说明隐藏目录排除机制和符号链接处理
- 添加了多道自动化门禁的详细说明，包括tsc编译检查、ESLint、OpenAPI同步等
- 完善了测试覆盖率提升85%的实现策略和实践方法
- 更新了故障排查指南，包含预推送门禁失败的解决方案
...
- **显著改进的测试基础设施**：测试文件从54个增长到90个，测试用例从569个增加到1035个，代表85%的覆盖率提升
...
- **单元测试**：与源码就近放置（如 server/*/*.test.ts、src/utils/*.test.ts、src/components/**/*.test.ts），由 Vite/Vitest 统一发现与运行
- **端到端测试**：集中在 tests/e2e 下，使用 Playwright 驱动真实浏览器进行冒烟与关键流程回归
- **评测与压测**：server/eval 提供评测集校验、LLM 准确率评测与连接池压测脚本
- **CI 质量门禁**：.github/workflows/ci.yml 定义多层级质量保障（类型检查、ESLint、OpenAPI 同步、状态巡检、Vitest 全量单测、评测集结构门禁）

```mermaid
graph TB
A["前端应用<br/>React + Vite"] --> B["后端服务<br/>Express + TS"]
B --> C["数据库<br/>MySQL/PG/Greenplum"]
D["Vitest 单测<br/>90个测试文件<br/>1035个测试用例"] --> A
D --> B
E["Playwright E2E<br/>冒烟+关键流程"] --> A
E --> B
F["评测/压测脚本<br/>准确率+性能"] --> B
G["CI 流水线<br/>多道自动化门禁"] --> D
G --> E
G --> F
H["隐藏目录排除<br/>.*/**"] --> D
I["tsc编译检查<br/>ESLint+OpenAPI"] --> G
```
...
## 核心组件
- **测试框架与工具链**
  - Vitest：用于前端与后端单元测试，排除 E2E 用例和隐藏目录，支持 jsdom 环境渲染 React 组件
...
- **多道自动化门禁**
  - TypeScript 类型检查（前端主 tsconfig + server strict + 前端 strictNullChecks）
  - ESLint 代码规范检查
  - OpenAPI 同步校验（代码路由与 docs/openapi.json 一致性）
  - 进程内状态巡检（白名单外新增模块级可变存储拦截）
  - Vitest 全量单测（90个测试文件，1035个测试用例）
  - 评测集结构门禁（规模≥100条 + 六类分层 + permission权限类覆盖）
...
participant VT as "Vitest<br/>90个测试文件"
...
### 单元测试（Vitest）
- **环境与配置**
  - 通过 vite.config.ts 的 test.exclude 将 E2E 用例和隐藏目录排除在 Vitest 之外，避免重复执行和误收集外部测试
  - **隐藏目录排除**：'.*/**' 模式排除 .agents/、.claude/、.qoder/ 等符号链接目录，防止 Vitest 跟随链接收集外部技能包测试
...
- **测试规模**
  - **90个测试文件**：涵盖服务端鉴权、知识管理、LLM客户端、查询执行、报告生成等核心模块
  - **1035个测试用例**：提供全面的边界条件、异常处理和性能场景覆盖
  - **85%覆盖率提升**：相比之前的54个测试文件和569个测试用例，实现了显著的覆盖率提升
...
```mermaid
flowchart TD
Start(["进入测试用例"]) --> ExcludeHidden["排除隐藏目录<br/>.*/**"]
ExcludeHidden --> MockDeps["Mock 外部依赖<br/>DB/Hooks/Utils"]
...
```mermaid
graph LR
Pkg["package.json 脚本"] --> VT["vitest<br/>90个测试文件"]
...
- **单测失败常见原因**
  ...
  - 组件测试环境缺失（需 jsdom 环境）
  - **预推送门禁失败** - Vitest 收集了外部技能包的 node:test 文件而非项目测试
    - 解决方案：确保 vite.config.ts 中 test.exclude 包含 '.*/**' 以排除隐藏目录
    - 影响范围：.agents/、.claude/、.qoder/ 等 agent 技能包目录
- **评测/压测失败**
  - 检查数据库连接与 Schema 初始化
  - 确认数据源类型与表存在，探针 SQL 可执行
- **CI 质量门禁失败**
  - TypeScript 类型检查：检查前端主 tsconfig、server strict、前端 strictNullChecks
  - ESLint 规范检查：修复代码风格问题
  - OpenAPI 同步：确保代码路由与 docs/openapi.json 一致
  - 状态巡检：检查白名单外的模块级可变存储
...
## 结论
本项目已形成较完整的测试体系：**测试基础设施显著改进**，测试文件从54个增长到90个，测试用例从569个增加到1035个，代表85%的覆盖率提升。以 Vitest 为核心的单元/集成测试保障代码正确性，以 Playwright 为主的 E2E 守护关键用户路径，辅以评测与压测脚本保障质量与稳定性。**多道自动化门禁**包括 tsc 编译检查、ESLint、OpenAPI 同步、状态巡检、Vitest 全量单测、评测集结构校验等，形成完整的质量保障体系。建议在团队内推广以下实践：
- 严格遵循测试金字塔，优先写高价值、低成本的单测
- 使用 Mock 隔离外部依赖，确保单测稳定与快速
- E2E 用例聚焦关键路径与回归防线，避免脆弱选择器
- 将覆盖率与评测阈值纳入 CI 门禁，持续改进质量
- **正确处理隐藏目录和符号链接**，避免外部技能包测试干扰项目测试
...
- **排除隐藏目录**：'.*/**' 模式排除 .agents/、.claude/、.qoder/ 等符号链接目录，防止 Vitest 跟随链接收集外部技能包测试
- **组件测试环境**：jsdom 用于 React 组件测试
- **覆盖率**：可通过 Vitest 参数启用，并在 CI 中收集报告
- **测试规模**：90个测试文件，1035个测试用例，85%覆盖率提升
...
- **本地执行**
  ...
  - 压测：npm run loadtest
- **CI 门禁**
  - **多道自动化门禁**：TypeScript 类型检查（前端+server双tsconfig）、ESLint、OpenAPI 同步、状态巡检、Vitest 全量单测
  - **评测集门禁**：结构校验（≥100条 + 六类覆盖 + permission权限类覆盖）、准确率阈值门禁
...
### 隐藏目录与符号链接处理
- **问题背景**：skills CLI 会将技能包装入 .agents 目录，并向 .claude/.qoder 等 agent 目录建立符号链接
- **影响**：Vitest glob 会跟随符号链接，收集到外部技能包的 node:test 文件
- **解决方案**：在 vite.config.ts 中添加 '.*/**' 排除规则，阻止 Vitest 进入隐藏目录
- **效果**：预推送门禁不再因外部测试文件而失败，只运行项目自身的测试

**章节来源**
- [vite.config.ts:36-42](file://vite.config.ts#L36-L42)

### 测试基础设施改进详情
- **测试文件增长**：从54个增长到90个，覆盖更多核心模块
- **测试用例扩展**：从569个增加到1035个，提供更全面的边界条件和异常处理覆盖
- **覆盖率提升**：85%的覆盖率提升，显著提升代码质量保障能力
- **多道自动化门禁**：
  - TypeScript 类型检查（前端主 tsconfig + server strict + 前端 strictNullChecks）
  - ESLint 代码规范检查
  - OpenAPI 同步校验
