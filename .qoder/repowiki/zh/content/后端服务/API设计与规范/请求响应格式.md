...
## 更新摘要
**所做更改**
- 新增高级分析能力模块（时序预测、多维归因、情景推演）
- 新增Agent编排能力模块（规划器+执行器）
- 扩展OpenAPI规范至159个一致端点
- 增强数据分析系统的智能化处理能力
...
## 简介
本文件面向"智能问数据分析系统"的 RESTful API 请求与响应格式，覆盖统一错误码、认证鉴权、数据源管理、自然语言问数主链路、报告导出、知识库、指标、权限申请、异步任务等。**最新更新**：新增高级分析能力（P1-5/6/9）和Agent编排能力（P1-7），包含时序预测、多维归因、情景推演以及多能力编排执行，将总API覆盖率提升至159个一致端点。文档基于 OpenAPI 3.0.3 规范定义（docs/openapi.json），并结合服务端路由实现（Express）说明请求体结构、响应体结构、分页约定、文件上传下载、流式响应处理、请求验证、数据转换、压缩策略以及版本兼容性管理。同时提供 API 测试工具、Mock 服务与契约测试的实施建议。

## 项目结构
- 入口与中间件：server.ts 负责 Express 应用初始化、全局中间件（JSON 解析大小限制、安全响应头、请求日志、Prometheus 指标）、路由挂载、健康检查、模型信息、兜底 404 与全局错误处理。
- 路由模块：按业务域拆分在 server/routes/*，如 auth、admin、datasources、query、conversation、report、export、knowledge、metrics、skills、accessRequests、tasks、**analytics、agent** 等。
...
A --> K["/api/analytics<br/>高级分析路由"]
A --> L["/api/agent<br/>Agent编排路由"]
M["docs/openapi.json<br/>OpenAPI 契约"] -.-> A
N["scripts/checkOpenapi.mjs<br/>契约校验"] -.-> M
```
...
### 高级分析能力（Analytics）
**新增功能** P1-5/6/9 高级分析能力，提供时序预测、多维归因和情景推演功能。

- 时序预测（P1-5）
  - POST /api/analytics/forecast：统计模型（移动平均/线性回归/季节分解，auto 回测择优）出未来 N 期 + 波动区间 + LLM 解读。
  - 请求体：yValues（必需，3~240期数值序列）、xValues（可选期次标签）、periods（1~24预测期数）、model（auto/ma/lr/seasonal）、metricLabel（指标名）、interpret（是否LLM解读）。
  - 响应：forecast（预测结果）、interpretation（LLM业务解读）。

- 多维归因（P1-6）
  - POST /api/analytics/attribution：按维度组合贡献度拆解（正=拉高/负=拉低）自动排序 + LLM 结论。
  - 请求体：rows（归因数据行）、aggregate（可选，服务端聚合映射）、subject（指标名）、interpret（是否LLM结论）。
  - 响应：attribution（归因结果）、periods（时间段）、interpretation（业务结论）。

- 情景推演（P1-9）
  - POST /api/analytics/whatif：LLM 按场景改写既有 SQL → 安全校验 → 与原 SQL 对比执行 → LLM 解读。
  - 请求体：dataSourceId（数据源ID）、sql（基准SQL）、scenario（情景描述≤500字）、question（原始提问背景）。
  - 响应：plan（改写计划）、before/after（前后SQL执行结果）、comparisons（对比结果）、summaryLines（总结）、interpretation（解读）。

```mermaid
sequenceDiagram
participant FE as "前端"
participant AN as "/api/analytics"
participant LLM as "LLM服务"
participant DB as "数据库"
FE->>AN : POST /forecast {yValues, periods}
AN->>LLM : 生成业务解读
LLM-->>AN : 解读结果
AN->>DB : 可选数据验证
DB-->>AN : 验证结果
AN-->>FE : {forecast, interpretation}
```
...
**章节来源**
- [analytics.ts:1-307](file://server/routes/analytics.ts#L1-307)
- [openapi.json:4598-4810](file://docs/openapi.json#L4598-L4810)

### Agent编排能力（Agent）
**新增功能** P1-7 Agent编排能力，提供多能力规划与顺序执行机制。

- 规划器（Planner）
  - POST /api/agent/plan：LLM Planner 生成多能力计划（query/forecast/attribution，只规划不执行，计划10分钟有效）。
  - 请求体：question（分析问题≤500字）、dataSourceId（数据源ID）。
  - 响应：plan（包含steps步骤的计划）。

- 执行器（Executor）
  - POST /api/agent/run：批准后顺序执行计划（query步走真实问数链路，forecast/attribution步对上游数据统计）。
  - 请求体：planId（计划ID）、dataSourceId（数据源ID）。
  - 响应：ok（执行状态）、traceId（追踪ID）、steps（执行步骤）、finalSummary（最终总结）。

```mermaid
flowchart TD
Plan["POST /api/agent/plan<br/>生成多能力计划"] --> Store["存储计划(10分钟有效)"]
Store --> Approve{"用户批准执行?"}
Approve --> |是| Run["POST /api/agent/run<br/>顺序执行计划"]
Approve --> |否| Expire["计划过期"]
Run --> QueryStep["query步骤：真实问数"]
Run --> ForecastStep["forecast步骤：时序预测"]
Run --> AttributionStep["attribution步骤：多维归因"]
QueryStep --> Execute["执行并返回结果"]
ForecastStep --> Execute
AttributionStep --> Execute
Execute --> Summary["生成最终总结"]
```
...
**章节来源**
- [agent.ts:1-115](file://server/routes/agent.ts#L1-115)
...
- **新增** 高级分析性能优化
  - 时序预测：支持最大240期历史数据，自动模型选择优化计算效率。
  - 多维归因：限制最大归因行数和维度数，避免内存溢出。
  - 情景推演：SQL长度限制（MAX_SCENARIO_SQL_LENGTH），结果行数限制（WHATIF_ROW_LIMIT）。
...
- **新增** 高级分析故障排查
  - 时序预测：检查yValues长度是否在3~240范围内，periods是否在1~24范围内。
  - 多维归因：验证rows数据结构，确保current和previous为数值型。
  - 情景推演：确认dataSourceId权限，检查SQL长度是否超过限制。
  - Agent编排：验证planId有效性（10分钟有效期），检查数据源类型是否支持。

**章节来源**
- [errorCodes.ts:1-36](file://server/infra/errorCodes.ts#L1-L36)
- [server.ts:267-280](file://server.ts#L267-L280)
- [query.ts:377-393](file://server/routes/query.ts#L377-L393)
- [analytics.ts:69-80](file://server/routes/analytics.ts#L69-L80)
- [analytics.ts:127-162](file://server/routes/analytics.ts#L127-L162)
- [analytics.ts:213-224](file://server/routes/analytics.ts#L213-L224)
- [agent.ts:24-35](file://server/routes/agent.ts#L24-L35)

## 结论
本系统采用统一的错误码与响应结构，结合严格的输入校验、权限控制、限流与并发控制，保障高可用与安全。**最新更新**：新增的高级分析能力（时序预测、多维归因、情景推演）和Agent编排能力（规划器+执行器）大幅增强了系统的智能化水平，将API覆盖率提升至159个一致端点。OpenAPI 契约与代码双向校验确保接口演进可控；SSE 流式与断线续传提升交互体验；缓存与审计机制兼顾性能与可观测性。建议在团队内持续维护 docs/openapi.json，并通过 scripts/checkOpenapi.mjs 在 CI 中强制执行契约一致性。
...
- **新增** 高级分析：
  - 时序预测：{ yValues, xValues?, periods?, model?, seasonalPeriod?, metricLabel?, interpret? }
  - 多维归因：{ rows, aggregate?, subject?, interpret? }
  - 情景推演：{ dataSourceId, sql, scenario, question? }
- **新增** Agent编排：
  - 规划：{ question, dataSourceId }
  - 执行：{ planId, dataSourceId }
...
- [openapi.json:133-161](file://docs/openapi.json#L133-L161)
- [openapi.json:4605-4655](file://docs/openapi.json#L4605-L4655)
- [openapi.json:4686-4726](file://docs/openapi.json#L4686-L4726)
- [openapi.json:4757-4784](file://docs/openapi.json#L4757-L4784)
- [openapi.json:4818-4837](file://docs/openapi.json#L4818-L4837)
- [openapi.json:4870-4889](file://docs/openapi.json#L4870-L4889)
...
- **新增** 高级分析验证：
  - 时序预测：yValues长度验证（3~240）、periods范围验证（1~24）、模型白名单校验。
  - 多维归因：rows数组验证、数值类型验证、维度数量限制。
  - 情景推演：SQL长度限制、数据源权限验证、SQL安全校验。
...
- [analytics.ts:69-84](file://server/routes/analytics.ts#L69-L84)
- [analytics.ts:127-167](file://server/routes/analytics.ts#L127-L167)
- [analytics.ts:213-224](file://server/routes/analytics.ts#L213-L224)
...
- **新增** 高级分析测试
  - 针对时序预测、多维归因、情景推演编写单元测试，验证边界条件和异常处理。
  - 使用Mock LLM服务测试解读功能，确保LLM调用失败时的降级处理。
  - 针对Agent编排测试规划器和执行器的完整流程。
