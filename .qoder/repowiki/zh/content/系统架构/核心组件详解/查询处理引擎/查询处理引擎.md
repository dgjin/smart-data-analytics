...
- **新增** 回退机制：fallbackPipeline、ruleBased、simplerPrompt、human_approval
...
- **新增** 回退对抗训练机制（fallbackPipeline）：三层策略降级，困难样本自动收集，人工标注流程，主动学习优先级排序。
...
- 主动学习排序：基于多因子评分模型对 PENDING 样本进行优先级排序
...
- Few-Shot 注入：被采纳的样本自动注入到 few-shot 服务，提升后续查询成功率
...
- **新增** 回退机制集成：[query.ts:294-387](file://server/routes/query.ts#L294-L387)、[fallbackPipeline.ts:47-138](file://server/utils/fallback/fallbackPipeline.ts#L47-L138)
- **新增** 人类审批流程：[fallbackApproval.ts:24-175](file://server/routes/fallbackApproval.ts#L24-L175)、[activeLearning.ts:40-129](file://server/utils/activeLearning.ts#L40-L129)
...
- [query.ts:294-387](file://server/routes/query.ts#L294-L387)
- [fallbackPipeline.ts:47-138](file://server/utils/fallback/fallbackPipeline.ts#L47-L138)
- [fallbackApproval.ts:24-175](file://server/routes/fallbackApproval.ts#L24-L175)
- [activeLearning.ts:40-129](file://server/utils/activeLearning.ts#L40-L129)
...
- **新增** 主动学习性能：优先级排序算法 O(n log n)，支持大规模样本高效处理；批量操作优化 SQL 长度限制。
...
## 结论
该查询处理引擎以"安全先行、性能优先、可观测可恢复"为原则，构建了从自然语言到 SQL、从计划到执行、从结果到解读的完整闭环。M3 中间表清洗链有效支撑复杂分析的复用与隔离；SQL 安全执行层提供多层防护与自愈能力；Schema 上下文与权限映射确保列级与行级访问控制；查询计划与模板机制兼顾灵活性与确定性。**新增的回退对抗训练机制进一步增强了系统的鲁棒性和持续学习能力**，通过三层策略降级、困难样本收集和人类审批流程，显著提升了复杂查询的处理成功率。主动学习优先级排序算法优化了标注队列管理，提高了人工审核效率。整体架构在多租户、多数据源、高并发场景下具备良好扩展性与稳定性。
