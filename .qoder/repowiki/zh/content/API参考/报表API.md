...
- 报表导出：PPT 导出、PDF 生成、CSV 自定义格式导出
- 实时报表的数据更新机制：数据源连接、定时刷新、增量更新（基于现有实现说明）
- 权限控制、分享与版本管理（基于现有实现说明）
- 导出任务状态跟踪、错误处理与大文件策略
- 完整的请求/响应示例（以路径与字段说明为主，不直接粘贴代码）

## 项目结构
报表相关能力由路由层、报告引擎、导出模块与异步任务队列共同组成：
- 路由层：负责鉴权、限流、输入校验、审计记录与结果返回
- 报告引擎：双阶段真实报表生成（计划→执行→文案），以及演示模式
- 导出模块：PPTX/PDF/CSV 构建器
...
异步任务：
- 提交即返回 taskId，worker 并发执行，心跳保活，孤儿回收，失败重试上限
- 文件类任务结果落盘，提供专用下载端点
...
C->>R : POST /api/report/export-pdf/async
R->>TQ : submitTask("report_export_pdf")
TQ->>TH : runExportPdf
TH->>EX : runPdfGenerator(...)
EX-->>TH : PDF Buffer
TH-->>TQ : completeTask({file : true,filename,size})
```
...
### 报表导出（PPT/PDF/CSV）
- PPT 导出：前端传 base64 PNG，服务端构建封面/摘要/KPI/每图一页/结论页，文件名安全化处理
- PDF 导出：调用 ReportLab 子进程，文本矢量排版，PNG 图表嵌入；支持横竖版；DLP 水印在服务端注入
- CSV 导出：统一通道，首尾行水印；超过阈值需管理员审批；硬上限防内存溢出
...
FE->>RP : POST /api/report/export (PPT)
RP->>RE : buildReportPptx(data)
RE-->>RP : Buffer
RP-->>FE : application/vnd.openxmlformats...
FE->>RP : POST /api/report/export-pdf (PDF)
RP->>PE : runPdfGenerator(data, orientation, watermark)
PE-->>RP : PDF Buffer
RP-->>FE : application/pdf
FE->>EX : POST /api/export/csv (rows, columns)
EX-->>FE : text/csv (含水印/审批)
```

**图示来源**
- [server/routes/report.ts:515-581](file://server/routes/report.ts#L515-L581)
- [server/report/reportExport.ts:66-222](file://server/report/reportExport.ts#L66-L222)
- [server/report/pdfExport.ts:66-132](file://server/report/pdfExport.ts#L66-L132)
...
- 定位步骤：
  - 检查审计日志 endpoint='report'/'export' 的 status 与 detail
...
## 结论
本系统提供了完整的报表生成与导出能力，涵盖模板管理、图表配置、数据绑定、PPT/PDF/CSV 导出、权限控制与异步任务管理。通过双阶段真实报表与严格的输入校验、审计与限流，保障了报表质量与安全性。对于实时性与增量更新，建议结合外部调度与应用层参数设计以满足不同场景需求。
