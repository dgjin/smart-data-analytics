...
## 更新摘要
**变更内容**
- 新增完整的优雅关闭系统，支持SIGTERM/SIGINT信号处理
- 实现可配置的排空序列和超时控制
- 增强数据库连接池关闭功能的幂等性
- 集成shutdown控制器到server.ts主应用
- 完善健康检查与优雅关闭的协调机制
...
- 优雅关闭系统（SIGTERM/SIGINT处理、可配置排空序列、资源清理）
...
## 项目结构
系统以 Express 为后端入口，提供 /api/health 健康检查与 /metrics Prometheus 指标；通过 Dockerfile 定义容器化运行方式；通过 docker-compose.multi-instance.yml 编排多实例；通过 Nginx 配置实现多实例负载均衡。**新增的优雅关闭系统**确保进程在收到终止信号时能够安全地停止服务。
...
- [server/infra/shutdown.ts:114-137](file://server/infra/shutdown.ts#L114-L137)
...
- **优雅关闭系统**
  - 监听 SIGTERM/SIGINT 信号，执行可配置的排空序列：停止后台任务 → 关闭新连接 → 等待在途请求完成 → 关闭资源 → 退出进程。
  - 支持超时控制和二次信号快速退出机制。
- **监控与可观测性**
...
## 架构总览
下图展示从客户端到应用、再到外部依赖的完整链路，以及监控、日志和优雅关闭系统的接入点。
...
participant Shutdown as "优雅关闭控制器"
...
App-->>Client : 响应/流式返回
Prom->>App : 抓取 /metrics
Note over Shutdown,App : SIGTERM/SIGINT触发优雅关闭
Shutdown->>App : beforeClose(停止任务worker)
Shutdown->>App : close(停止接收新连接)
Shutdown->>App : closeIdleConnections(回收空闲连接)
Shutdown->>App : closeResources(关闭数据库连接池)
```
...
### 优雅关闭系统（新增）
**新增** 系统实现了完整的优雅关闭机制，确保进程在终止时能够安全地释放所有资源。

- **信号处理**
  - 监听 SIGTERM 和 SIGINT 信号，首次信号启动优雅关闭流程，二次信号立即退出防止卡死。
  - 通过 `installGracefulShutdown` 函数注册信号处理器，返回控制器和清理函数。
- **排空序列**
  - `beforeClose`: 停止后台任务领取器（如任务worker），不再接受新任务。
  - `server.close`: 停止接收新的HTTP连接。
  - `closeIdleConnections`: 立即回收空闲的keep-alive连接。
  - 等待在途请求自然结束（限时 SHUTDOWN_TIMEOUT_MS）。
  - 超时后强制关闭剩余连接。
  - `closeResources`: 关闭数据库连接池等资源。
  - `exit(0)`: 正常退出进程。
- **超时控制**
  - 通过 `SHUTDOWN_TIMEOUT_MS` 环境变量配置排空超时时间，默认10秒，范围限制在1-120秒。
  - 超时后强制关闭连接，确保进程不会无限期挂起。
- **幂等性保证**
  - drain过程中重复触发直接返回，避免重复执行关闭逻辑。
  - 数据库连接池关闭函数 `closePool` 具有幂等性，未初始化时静默返回。

```mermaid
flowchart TD
Signal[收到SIGTERM/SIGINT] --> Check{是否已在drain中?}
Check --> |是| Exit1[立即exit(1)]
Check --> |否| BeforeClose[beforeClose: 停止任务worker]
BeforeClose --> CloseServer[server.close: 停止接收新连接]
CloseServer --> CloseIdle[closeIdleConnections: 回收空闲连接]
CloseIdle --> WaitWait{等待在途请求}
WaitWait --> Timeout{超时?}
Timeout --> |是| ForceClose[closeAllConnections: 强制关闭]
Timeout --> |否| Resources[closeResources: 关闭数据库连接池]
ForceClose --> Resources
Resources --> Exit0[exit(0): 正常退出]
```
...
章节来源
- [server/infra/shutdown.ts:1-138](file://server/infra/shutdown.ts#L1-138)
- [server.ts:330-340](file://server.ts#L330-L340)
- [server/infra/db.ts:53-62](file://server/infra/db.ts#L53-L62)
...
- **systemd 服务配置建议**
  ...
  - **优雅关闭支持**：systemd 发送 SIGTERM 信号，应用会执行优雅关闭流程。
...
- **HTTP 健康端点**
  ...
  - **新增** /api/health/live 浅探测，/api/health/ready 深探测（包含数据库和Redis连接检查）。
  - **优雅关闭集成**：drain期间 /api/health/ready 返回503，让上游摘除流量。
- **数据库连接检查**
  - 启动时执行 initSchema，失败会阻断启动；后续业务逻辑中如连接异常应走降级路径。
  - **改进** closePool 函数支持幂等关闭，优雅关闭时安全释放连接池。
- **Redis 连接状态监控**
...
Note over Shutdown,App : drain期间/health/ready返回503
...
- **优雅关闭流程**
  - **新增** 完整的优雅关闭机制，确保进程安全退出：
    - 停止接受新请求，等待进行中请求完成
    - 关闭数据库连接池、Redis连接、LLM连接等资源
    - 支持超时控制和二次信号快速退出
    - 健康检查在drain期间返回503，让上游摘除流量
...
- **优雅关闭依赖**
  - **新增** shutdown控制器依赖http.Server接口，支持close、closeIdleConnections、closeAllConnections方法。
  - 依赖logger模块记录优雅关闭过程日志。
...
**图表来源**
- [server.ts:98-131](file://server.ts#L98-L131)
- [server/infra/shutdown.ts:14-35](file://server/infra/shutdown.ts#L14-L35)
- [server/infra/monitoring.ts:16-18](file://server/infra/monitoring.ts#L16-L18)
- [server/infra/logger.ts:1-41](file://server/infra/logger.ts#L1-L41)
...
- **优雅关闭性能**
  - **新增** 可配置的超时控制避免长时间阻塞；空闲连接立即回收减少等待时间；超时后强制关闭确保进程及时退出。
...
- [server/infra/shutdown.ts:44-48](file://server/infra/shutdown.ts#L44-L48)

## 故障排查指南
- **健康检查失败**
  - 检查 /api/health 是否可达；确认容器 HEALTHCHECK 配置与探针间隔/重试次数。
  - **新增** 检查 /api/health/ready 在drain期间的行为，确认优雅关闭正常工作。
- **指标不可用**
  - 检查 /metrics 是否被 METRICS_TOKEN 保护；确认 Prometheus job 配置与网络连通。
- **日志过多或过少**
  - 调整 LOG_LEVEL；在测试环境非 error 日志会被静默。
  - **新增** 检查优雅关闭相关日志，确认关闭流程是否正常执行。
- **多实例不一致**
  - 确认 Redis 连接一致；确认 JWT_SECRET 在所有实例相同；确认 Nginx upstream 列表正确。
- **优雅关闭问题**
  - **新增** 检查 SHUTDOWN_TIMEOUT_MS 环境变量配置是否合理。
  - 查看优雅关闭日志，确认各个阶段是否正常执行。
  - 确认数据库连接池是否正确关闭，避免连接泄漏。
...
## 结论
本系统采用无状态应用设计，通过 Redis 共享状态、Nginx 负载均衡与 Prometheus 监控，实现了可扩展、可观测的多实例部署。**新增的优雅关闭系统**进一步增强了系统的可靠性，确保进程在终止时能够安全地释放所有资源，避免数据丢失和服务中断。健康检查与依赖健康探测保障了服务的稳定性。结合 PM2/systemd 的进程管理策略，可实现自动重启、资源限制与优雅关闭。日志统一出口与轮转建议有助于长期稳定运行与问题定位。
...
- **环境变量要点**
  ...
  - **新增** SHUTDOWN_TIMEOUT_MS：优雅关闭超时时间（默认10秒，范围1-120秒）。
- **启动脚本**
  - package.json scripts 提供 dev/build/start 命令；Dockerfile 指定 CMD 运行 dist/server.cjs。
- **优雅关闭配置**
  - **新增** 通过 `installGracefulShutdown` 函数配置优雅关闭行为，支持beforeClose和closeResources钩子。
  - 可通过环境变量SHUTDOWN_TIMEOUT_MS调整排空超时时间。
