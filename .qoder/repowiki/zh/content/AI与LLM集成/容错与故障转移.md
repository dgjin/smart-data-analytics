...
## 更新摘要
**变更内容**
- 新增 A/B 测试集成机制，支持回退策略效果对比分析
- 增强主动学习优先级排序算法，优化困难样本处理效率
- 完善少样本注入能力，实现专家修正 SQL 的自动知识库构建
- 扩展回退机制监控指标，提供完整的策略效果评估体系
- 更新数据库表结构，支持新的实验记录和审计日志功能
...
- **增强版** NL2SQL 回退机制：三层策略降级、A/B 测试集成、主动学习优先级排序、少样本注入能力
...
subgraph "A/B 测试框架"
H["abTest.ts<br/>实验分组/统计分析"]
I["routes/abTest.ts<br/>API 接口"]
J["fallback_ab_tests<br/>实验数据表"]
end
subgraph "主动学习系统"
K["activeLearning.ts<br/>优先级排序算法"]
L["adversarial_samples<br/>困难样本库"]
end
subgraph "少样本注入服务"
M["fewShotService.ts<br/>知识库注入/检索"]
N["knowledge_base<br/>Few-Shot 知识库"]
end
subgraph "基础设施"
O["rateLimiter.ts<br/>IP级限流(内存/Redis)"]
P["taskQueue.ts<br/>长任务队列/心跳/孤儿回收"]
Q["monitoring.ts<br/>Prometheus 指标埋点"]
R["stateStore.ts<br/>分布式状态/锁/窗口计数"]
S["logger.ts<br/>统一日志"]
T["llmUsage.ts<br/>用量落库+指标旁路"]
U["auditLog.ts<br/>审计日志(含回退信息)"]
V["db.ts<br/>数据库迁移(回退字段)"]
end
A --> B
A --> C
A --> Q
...
- [fewShotService.ts:28-76](file://server/utils/fewShotService.ts#L28-L76)
- [db.ts:729-777](file://server/infra/db.ts#L729-L777)
...
- [activeLearning.ts:40-129](file://server/utils/activeLearning.ts#L40-L129)
- [fewShotService.ts:28-119](file://server/utils/fewShotService.ts#L28-L119)
...
- [activeLearning.ts:40-129](file://server/utils/activeLearning.ts#L40-L129)
- [fewShotService.ts:81-119](file://server/utils/fewShotService.ts#L81-L119)
...
3. 插入到 knowledge_base 表中，标记为 FEW_SHOT 类型
4. 记录注入日志和统计信息
...
Service->>KB : INSERT knowledge_base
KB-->>Service : 确认插入
end
...
**章节来源**
- [fewShotService.ts:1-119](file://server/utils/fewShotService.ts#L1-L119)
...
- **新增** 主动学习问题排查：
  ...
  - 处理效率低下：调整批次大小和并发度设置
- **新增** 少样本注入问题排查：
  - 注入失败：检查 knowledge_base 表结构和插入权限
  - 检索结果不准确：验证数据源过滤和排序逻辑
