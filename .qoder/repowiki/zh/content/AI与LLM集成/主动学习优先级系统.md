...
- [server/utils/activeLearning.test.ts](file://server/utils/activeLearning.test.ts)
</cite>

## 更新摘要
**变更内容**
- 修复了用户ID列映射问题（snake_case到camelCase转换）
- 实现了基于实际用户失败频率的优先级评分机制
- 修正了整个主动学习管道中的字段名处理
- 增强了错误处理和异常保护机制
- 优化了批量操作和审核流程的原子性
...
章节来源
- [server/utils/activeLearning.ts:11-164](file://server/utils/activeLearning.ts#L11-L164)
...
**已更新** 修复了关键的用户ID列映射问题和基于实际用户失败频率的优先级评分机制

- 关键实现要点
  - SQL 模板标准化：去除数字与字符串常量，便于统计重复度
  - 异常保护：捕获异常返回空数组，避免阻塞上游
  - 日志记录：记录样本数量、最高分与平均分
  - **新增** 用户失败频率统计：统计每个用户的待审失败次数，高频用户样本获得更高权重
  - **新增** snake_case到camelCase显式映射：确保API响应字段正确映射
...
Q --> |无数据| End(["结束"])
CalcFreq --> UserFreq["统计各用户失败频率"]
UserFreq --> Normalize["标准化 SQL 模板"]
Normalize --> Score["逐样本计算五维分数"]
Score --> MapFields["映射 snake_case 到 camelCase"]
MapFields --> Sort["按总分降序排序并填充排名"]
Sort --> Log["记录统计信息"]
Log --> End
```
...
**已更新** 修复了批量操作的参数对齐问题和审核流程的原子性

- 关键改进
  - **新增** 批量操作参数重建：filter后重新构建占位符，避免参数错位
  - **新增** 单步原子UPDATE：合并两步非原子操作，防止并发冲突
  - **新增** 缺失参数检查：修复SELECT语句缺少参数的500错误
...
## 结论
主动学习优先级系统将"失败样本 → 优先级排序 → 人工审核 → 知识注入"形成闭环，显著提升 Few-Shot 知识库质量与系统鲁棒性。通过多维度评分与管理端工作流，确保高价值样本优先处理，降低人工成本，加速模型迭代。

**已更新** v0.9.47版本的关键bug修复显著提升了系统的稳定性和准确性：
- 用户ID列映射修复确保了优先级评分的正确性
- 基于实际用户失败频率的评分机制提高了高价值样本的识别能力
- 字段名处理的规范化保证了API响应的完整性
- 批量操作和审核流程的原子性增强了系统的并发安全性

未来可进一步优化评分权重、扩展策略与增强前端交互体验。
