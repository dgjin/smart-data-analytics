...
/** 领取查询行（async_tasks 表列投影；payload_json 为 JSON 文本） */
interface ClaimedTaskRow extends mysql.RowDataPacket {
  id: string;
  type: TaskType;
...
/** 孤儿巡检行（async_tasks 表列投影） */
interface OrphanTaskRow extends mysql.RowDataPacket {
  id: string;
  attempts: number;
}
...
export async function submitTask(
  ...
  if (Number(rows[0]?.cnt) >= taskUserQueueMax()) return null;
...
export async function claimNextTask(workerId: string, pool?: mysql.Pool): Promise<{ id: string; type: TaskType; payload: any } | null> {
  ...
  try {
    ...
    const [rows] = await conn.query<ClaimedTaskRow[]>(
...
export async function recoverOrphanTasks(pool?: mysql.Pool, timeoutMs = taskHeartbeatTimeoutMs()): Promise<number> {
  const p = pool ?? getPool();
  const [rows] = await p.query<OrphanTaskRow[]>(
...
export async function getTask(taskId: string, pool?: mysql.Pool): Promise<AsyncTask | null> {
  const p = pool ?? getPool();
  const [rows] = await p.query<mysql.RowDataPacket[]>('SELECT * FROM async_tasks WHERE id = ?', [taskId]);
  const row = rows[0];
