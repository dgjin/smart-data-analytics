/**
 * P0-2 Word 导出单测：DOCX 生成 smoke（含无图兜底）、伪造载荷防御。
 */
import { describe, it, expect } from 'vitest';
import { buildReportWord } from './reportExportWord';
import { normalizeExportData } from './reportExport';

const PNG = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==';

describe('buildReportWord', () => {
  it('完整报告生成 DOCX（ZIP 魔数校验）', async () => {
    ...
    const buf = await buildReportWord(data);
  ...
  it('伪造图片载荷（声明 PNG 实为 ICNS）走文字兜底且产物仍为有效 DOCX', async () => {
