...
import { approxTokens } from '../llm/promptBudget';
...
/** 保留窗口计数行 */
interface CntRow extends mysql.RowDataPacket {
  cnt: number;
}
...
export async function pruneConversationHistory(userId: number, dataSourceId: string): Promise<number> {
  ...
  if (keep <= 0) return 0;
  const [cntRows] = await getPool().query<CntRow[]>(
    'SELECT COUNT(*) AS cnt FROM conversation_history WHERE user_id = ? AND data_source_id = ?',
    [userId, dataSourceId]
  );
  const excess = Number(cntRows[0]?.cnt || 0) - keep;
...
export async function searchConversations(
  ...
  const [rows] = kw
    ? await getPool().query<mysql.RowDataPacket[]>(
...
/** 个人 few-shot 检索行：本人真实执行成功的问答对 */
interface FewShotRow extends mysql.RowDataPacket {
  question: string;
  executed_sql: string;
}
...
export async function loadConversationFewShot(
  ...
  const [rows] = await getPool().query<FewShotRow[]>(
