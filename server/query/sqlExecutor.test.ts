...
describe('EXPLAIN 防线：explainGuardMaxRows 阈值配置', () => {
  afterEach(() => {
    delete process.env.SQL_EXPLAIN_MAX_ROWS;
  });

  it('默认 100 万行；export 场景放宽 10 倍', () => {
    expect(explainGuardMaxRows()).toBe(1_000_000);
    expect(explainGuardMaxRows('interactive')).toBe(1_000_000);
    expect(explainGuardMaxRows('chain')).toBe(1_000_000);
    expect(explainGuardMaxRows('export')).toBe(10_000_000);
  });

  it('SQL_EXPLAIN_MAX_ROWS 显式配置生效（export 同比放大）', () => {
    process.env.SQL_EXPLAIN_MAX_ROWS = '50000';
    expect(explainGuardMaxRows()).toBe(50_000);
    expect(explainGuardMaxRows('export')).toBe(500_000);
  });

  it('0 = 关闭防线；非法值回退默认', () => {
    process.env.SQL_EXPLAIN_MAX_ROWS = '0';
    expect(explainGuardMaxRows()).toBe(0);
    expect(explainGuardMaxRows('export')).toBe(0);
    process.env.SQL_EXPLAIN_MAX_ROWS = 'abc';
    expect(explainGuardMaxRows()).toBe(1_000_000);
    process.env.SQL_EXPLAIN_MAX_ROWS = '-5';
    expect(explainGuardMaxRows()).toBe(1_000_000);
  });
});

describe('EXPLAIN 防线：parseMysqlExplainRows 解析', () => {
  it('多表 JOIN 取各表 rows 最大值（预估检查行数）', () => {
    const rows = [
      { id: 1, select_type: 'SIMPLE', table: 'clients', rows: 5_200_000 },
      { id: 1, select_type: 'SIMPLE', table: 'visits', rows: 120_000 },
    ];
    expect(parseMysqlExplainRows(rows)).toBe(5_200_000);
  });

  it('rows 为 null 的行（派生表/物化）跳过不误判', () => {
    const rows = [
      { id: 1, table: '<derived2>', rows: null },
      { id: 1, table: 'clients', rows: 80 },
    ];
    expect(parseMysqlExplainRows(rows)).toBe(80);
  });

  it('空结果/非数组/全 null 返回 0（不拦截）', () => {
    expect(parseMysqlExplainRows([])).toBe(0);
    expect(parseMysqlExplainRows(null as any)).toBe(0);
    expect(parseMysqlExplainRows([{ rows: null }])).toBe(0);
  });

  it('FORMAT=JSON MySQL 8.0 传统结构：递归取 table 节点 rows_examined_per_scan 最大值', () => {
    const json = JSON.stringify({
      query_block: {
        ...
        grouping_operation: {
          ...
          table: { table_name: 'clients', access_type: 'ALL', rows_examined_per_scan: 2_500_000, rows_produced_per_join: 60, cost_info: { read_cost: '1.0' } },
        },
      },
    });
    expect(parseMysqlExplainRows([{ EXPLAIN: json }])).toBe(2_500_000);
  });

  it('FORMAT=JSON MySQL 9.x 新结构：递归 inputs 取 estimated_rows 最大值', () => {
    const json = JSON.stringify({
      query: 'select ...',
      query_plan: {
        limit: 500,
        inputs: [
          { inputs: [{ operation: 'Table scan on clients', table_name: 'clients', estimated_rows: 60.0, estimated_total_cost: 6.25 }] },
        ],
      },
    });
    expect(parseMysqlExplainRows([{ EXPLAIN: json }])).toBe(60);
  });

  it('树形文本（部分版本默认输出）：正则提取 rows=N 最大值', () => {
    const tree = '-> Limit: 500 row(s)\n    -> Table scan on <temporary>\n        -> Aggregate using temporary table\n            -> Table scan on clients  (cost=6.25 rows=60)\n';
    expect(parseMysqlExplainRows([{ EXPLAIN: tree }])).toBe(60);
    const bigTree = '-> Table scan on fct_main  (cost=100.25 rows=2800000)\n    -> Index scan on idx  (rows=500)';
    expect(parseMysqlExplainRows([{ EXPLAIN: bigTree }])).toBe(2_800_000);
  });

  it('畸形 JSON 文本回退树形正则', () => {
    expect(parseMysqlExplainRows([{ EXPLAIN: '{broken json... rows=123' }])).toBe(123);
  });
});

describe('EXPLAIN 防线：parsePgExplainRows 解析', () => {
  const pgPlan = (root: any) => [{ 'QUERY PLAN': [{ Plan: root }] }];

  it('聚合 + 单表 Seq Scan：取扫描节点 Plan Rows', () => {
    const rows = pgPlan({
      'Node Type': 'Aggregate',
      'Plan Rows': 1,
      Plans: [{ 'Node Type': 'Seq Scan', 'Relation Name': 'fct_main', 'Plan Rows': 2_800_000 }],
    });
    expect(parsePgExplainRows(rows)).toBe(2_800_000);
  });

  it('嵌套计划树递归取最大扫描节点（Index Scan 与 Seq Scan 混合）', () => {
    const rows = pgPlan({
      'Node Type': 'Nested Loop',
      'Plan Rows': 500,
      Plans: [
        { 'Node Type': 'Index Scan', 'Relation Name': 'clients', 'Plan Rows': 50 },
        {
          'Node Type': 'Bitmap Heap Scan',
          'Relation Name': 'visits',
          'Plan Rows': 900_000,
          Plans: [{ 'Node Type': 'Bitmap Index Scan', 'Relation Name': 'visits_pkey', 'Plan Rows': 950_000 }],
        },
      ],
    });
    expect(parsePgExplainRows(rows)).toBe(950_000);
  });

  it('非扫描节点（Sort/Aggregate/CTE 引用）不计入；空/畸形输入返回 0', () => {
    const rows = pgPlan({ 'Node Type': 'Sort', 'Plan Rows': 9_999_999, Plans: [{ 'Node Type': 'Index Only Scan', 'Relation Name': 't', 'Plan Rows': 42 }] });
    expect(parsePgExplainRows(rows)).toBe(42);
