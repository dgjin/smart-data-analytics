...
export function validateTemplateParams<T>(id: TemplateId, params: unknown): { ok: true; params: T } | { ok: false; reason: string } {
  ...
  // 外部（LLM/请求体）传入的模板参数按 unknown 逐字段校验，通过后原样交由调用方泛型 T 约束
  const t = params as Record<string, unknown>;
  ...
  switch (id) {
    ...
    case 'top_n_with_pct': {
      // 数组放宽为 unknown[] 以接收未收窄的 sortBy（非字符串值运行时自然判 false）
      if (!t.metricAlias || !t.dimension || !Number.isInteger(t.topN) || !(['ASC', 'DESC'] as unknown[]).includes(t.sortBy)) return { ok: false, reason: '缺少必要参数或排序方向无效' };
      return { ok: true, params: t as T };
    }
