...
export type ExecOutcome =
  ...
  | { ok: false; reason: string; /** EXPLAIN 防线拦截（预估扫描量超阈值）：调用方可识别此标记做定向呈现，而非普通执行失败 */ guardBlocked?: boolean };

// ---------- EXPLAIN 防线（P0：防大扫描拖垮业务库） ----------

/** 防线阈值：预估扫描行数超过该值则拦截。环境变量 SQL_EXPLAIN_MAX_ROWS（默认 100 万；0=关闭防线） */
export function explainGuardMaxRows(scenario: QueryScenario = 'interactive'): number {
  const raw = Number(process.env.SQL_EXPLAIN_MAX_ROWS);
  const base = Number.isFinite(raw) && raw >= 0 ? Math.floor(raw) : 1_000_000;
  if (base === 0) return 0;
 ...
 *  兼容两代格式：MySQL 8.0 为 table 节点级 rows_examined_per_scan；MySQL 9.x 为节点级 estimated_rows */
function mysqlJsonScanRows(node: unknown): number {
  if (!node || typeof node !== 'object') return 0;
  // 单次受控断言：上方守卫已排除 null/标量，EXPLAIN JSON 为驱动返回的动态结构
  const obj = node as Record<string, unknown>;
  let max = 0;
  const examined = Number(obj.rows_examined_per_scan);
  if (Number.isFinite(examined) && examined > max) max = examined;
  const estimated = Number(obj.estimated_rows);
  if (Number.isFinite(estimated) && estimated > max) max = estimated;
  for (const v of Object.values(obj)) {
    if (v && typeof v === 'object') {
      const r = mysqlJsonScanRows(v);
      if (r > max) max = r;
    }
  }
  return max;
...
/** MySQL EXPLAIN 结果 → 预估扫描行数（取各表最大值）。兼容三种返回形态：
 *  1. 传统表格（多列对象数组，rows 字段）2. FORMAT=JSON（单列 EXPLAIN，内为 JSON 文本）
 *  3. 树形文本（MySQL 8.0.16+ 部分驱动/版本默认，单列 EXPLAIN，含 "rows=N" 标注） */
export function parseMysqlExplainRows(explainRows: Record<string, unknown>[]): number {
  const list = Array.isArray(explainRows) ? explainRows : [];
  let max = 0;
  // 形态 1：传统表格多列（含数值 rows 字段）
  for (const r of list) {
    const v = Number(r?.rows);
    if (Number.isFinite(v) && v > max) max = v;
  }
  if (max > 0) return max;
  // 形态 2/3：单列 EXPLAIN 文本（JSON 或树形）
  const text = list.length > 0 ? list[0]?.EXPLAIN : null;
  if (typeof text !== 'string' || text.trim() === '') return max;
  const trimmed = text.trim();
  if (trimmed.startsWith('{')) {
    try {
      max = mysqlJsonScanRows(JSON.parse(trimmed));
    } catch { /* 畸形 JSON 落到树形正则 */ }
  }
  if (max > 0) return max;
  // 树形文本：提取所有 "rows=N" 标注取最大（如 "-> Table scan on clients (cost=6.25 rows=60)"）
  const re = /rows=(\d+(?:\.\d+)?)/gi;
  let m: RegExpExecArray | null;
...
/** 递归遍历 PG 计划树，取扫描节点的预估行数最大值（节点类型以 Scan 结尾，含 Seq/Index/Bitmap/Foreign 等） */
function pgScanRows(node: unknown): number {
  if (!node || typeof node !== 'object') return 0;
  // 单次受控断言：上方守卫已排除 null/标量，PG 计划树为驱动返回的动态结构
  const obj = node as Record<string, unknown>;
  let max = 0;
  const type = String(obj['Node Type'] || '');
  if (/Scan$/i.test(type)) {
    const v = Number(obj['Plan Rows']);
    if (Number.isFinite(v) && v > max) max = v;
  }
  const plans = obj.Plans;
  for (const child of Array.isArray(plans) ? plans : []) {
    const v = pgScanRows(child);
    if (v > max) max = v;
  }
  return max;
}

/** PG EXPLAIN (FORMAT JSON) 结果 → 预估扫描行数（扫描节点 Plan Rows 最大值） */
export function parsePgExplainRows(explainRows: Record<string, unknown>[]): number {
  const first = Array.isArray(explainRows) ? explainRows[0] : null;
  const planWrapper = first && typeof first === 'object' ? first['QUERY PLAN'] : null;
  const root = Array.isArray(planWrapper) ? planWrapper[0] : null;
  // 单次受控断言：计划树节点为 PG 驱动返回的动态结构，typeof 守卫后仅取 Plan 字段
  return pgScanRows(root && typeof root === 'object' ? (root as Record<string, unknown>).Plan : null);
}

/**
 * EXPLAIN 防线：校验/行过滤注入通过后、真执行前，先 EXPLAIN 评估扫描量。
 * - 超阈值：返回拦截原因（带 guardBlocked 标记），不执行
 * - EXPLAIN 自身失败：fail-open 放行（防线尽力而为，不因权限/方言差异阻断正常查询），埋点 error
 * 口径说明：MySQL rows=预计检查行数（准确）；PG Plan Rows=扫描节点预估输出行数（过滤后估计，
 * 强过滤大表可能低估，但无过滤的大范围聚合——最主要的风险场景——能准确拦截）。
 */
async function explainGuard(
  entry: DsPoolEntry,
  finalSql: string,
  ...
  try {
    let estimated = 0;
    if (entry.dialect === 'pg') {
      const result = await entry.pool.query<Record<string, unknown>>(`EXPLAIN (FORMAT JSON) ${finalSql}`);
      estimated = parsePgExplainRows(result.rows || []);
    } else {
      // FORMAT=JSON 跨版本结构稳定（8.0: rows_examined_per_scan / 9.x: estimated_rows），解析器两代兼容
      const [rows] = await entry.pool.query<mysql.RowDataPacket[]>(`EXPLAIN FORMAT=JSON ${finalSql}`);
      estimated = parseMysqlExplainRows(rows || []);
    }
    if (estimated > maxRows) {
      observeExplainGuard('blocked', entry.dialect);
      const fmt = (n: number) => (n >= 10000 ? `${(n / 10000).toFixed(1)} 万` : `${n}`);
      return {
        blocked: true,
        reason: `EXPLAIN_GUARD: 该查询预估扫描约 ${fmt(estimated)} 行，超过安全阈值 ${fmt(maxRows)} 行，为保护数据源性能已拦截。请增加时间范围、部门或维度等筛选条件收窄查询范围后重试。`,
      };
    }
...
/** data_sources 配置读取行（config_json 落库 JSON 文本） */
interface DataSourceConfigRow extends mysql.RowDataPacket {
  type: string;
...
export async function loadDataSourceConfig(dataSourceId: string): Promise<{ type: string; config: any } | null> {
  ...
  const ds = rows[0];
  if (!ds) return null;
  let config: Record<string, unknown> | string | null = ds.config_json;
  if (typeof config === 'string') {
    // 单次受控断言：config_json 为落库 JSON 文本，解析结果按配置对象口径使用（畸形 JSON 落 catch 回退空对象）
    try { config = JSON.parse(config) as Record<string, unknown>; } catch { config = {}; }
  }
  config = config || {};
