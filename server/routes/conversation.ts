...
router.get('/', authMiddleware, requireRole('ADMIN', 'ANALYST'), async (req, res) => {
  ...
  if (!dataSourceId) return res.status(400).json({ code: ERROR_CODES.INVALID_INPUT, error: '缺少数据源' });
...
router.delete('/:id', rateLimiter, authMiddleware, requireRole('ADMIN', 'ANALYST'), async (req, res) => {
  ...
  if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ code: ERROR_CODES.INVALID_INPUT, error: '对话记录 ID 无效' });
  try {
