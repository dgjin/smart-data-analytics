...
/** 预置标记查询行：update / delete 前的预置条目防护检查 */
interface KbEntryPresetRow extends RowDataPacket {
  is_preset: number;
}
...
export async function updateKnowledgeEntry(
  ...
  // 检查是否为预置条目（禁止修改）
  const [presets] = await getPool().query<KbEntryPresetRow[]>(
  ...
  const [result] = await getPool().query<ResultSetHeader>(sql, values);
