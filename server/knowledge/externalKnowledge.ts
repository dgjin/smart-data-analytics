...
export async function searchExternalKnowledge(
  ...
  try {
    ...
    const sources = rows.map(normalizeRow);
...
export async function listExternalKbSources(): Promise<(Omit<ExternalKbSource, 'apiKey'> & { hasKey: boolean })[]> {
  const [rows] = await getPool().query<mysql.RowDataPacket[]>('SELECT * FROM external_kb_sources ORDER BY created_at DESC');
  return rows.map((r) => {
...
export async function deleteExternalKbSource(id: string): Promise<boolean> {
  const [result] = await getPool().query<mysql.ResultSetHeader>('DELETE FROM external_kb_sources WHERE id = ?', [id]);
